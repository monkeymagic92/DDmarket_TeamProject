...
public class UserController {
	...
	public String myPage(UserPARAM param, Model model, HttpServletRequest request,
		...
		if(i_tap == 1) {
			...
			cperPageNum = 6;
			Paging.getPage(service.selMyBuyCnt(bparam), bparam, page, cperPageNum, model, request, hs);			
			model.addAttribute("myBuyList", service.selMyBuyList(bparam));
		} else if(i_tap == 4) {
			cperPageNum = 6;
	...
	public String info(Model model, UserPARAM param, HttpSession hs, HttpServletRequest request) {
	...
	public String info(Model model, UserVO vo, UserPARAM param,
		...
		if(chk == 0) { // 메소드 실행안됬을 경우
			...
			ra.addFlashAttribute("serverErr", "관심사는 총 1개를 선택해주세요");
			return "redirect:/" + ViewRef.USER_INFO;
		}
